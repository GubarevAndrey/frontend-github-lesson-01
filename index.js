...
JOKE_CARD.addEventListener("submit", async (event) => {  //async -это значить что функция ассинхронная
  ...
  const response = await fetch(RANDON_JOKE_URL, { // чтобы в respose пришел не Promise(обещание), а ответ
    method: "GET",                                // ставим await-будет ждать пока не получим ответ
  });
  ...
  const result = await response.json(); // чтобы в respose пришел не Promise(обещание), а ответ
               // ставим await-будет ждать пока не получим ответ
